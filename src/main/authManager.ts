...
export class AuthManager {
  ...
  constructor(store: Store) {
    ...
    this.migrateLegacyAccountLabels();
  }

  // One-time migration: accounts created before the Meta Ads rebrand were
  // labeled "Instagram Account" / "Instagram Profile" by default. Rename any
  // that still have those placeholder labels so old local data matches the
  // current app.
  private migrateLegacyAccountLabels(): void {
    const legacyLabels = new Set(['Instagram Account', 'Instagram Profile']);
    const accounts = this.store.get<WorkspaceAccount[]>('workspaceAccounts') || [];
    let changed = false;
    const migrated = accounts.map(account => {
      if (legacyLabels.has(account.label)) {
        changed = true;
        return { ...account, label: 'Ad Account' };
      }
      return account;
    });
    if (changed) {
      this.store.set('workspaceAccounts', migrated);
    }
  }
  ...
  // Workspace accounts (ad accounts)
  addWorkspaceAccount(workspaceId: string, label: string, email?: string): WorkspaceAccount | { error: string } {
    ...
    const colors = ['#833ab4', '#C13584', '#E1306C', '#F77737', '#FCAF45', '#5851DB', '#405DE6', '#26A69A'];
